CREATE TABLE work_overlap_document_targets (
    ...
    CONSTRAINT uq_work_overlap_document_targets UNIQUE (work_duty_id, bid_seq, engr_id, contract_no)
);

CREATE INDEX IF NOT EXISTS ix_work_overlap_document_targets_work_duty_bid_engr
    ON work_overlap_document_targets (work_duty_id, bid_seq, engr_id);
...
COMMENT ON COLUMN work_overlap_document_targets.engr_id IS '문서 생성 대상 기술인 식별자';
