CREATE TABLE IF NOT EXISTS work_overlap_contract_engineers (
    ...
    CONSTRAINT pk_work_overlap_contract_engineers
        PRIMARY KEY (contract_no, engineer_id),
...
CREATE INDEX IF NOT EXISTS ix_work_overlap_contract_engineers_engineer_id
    ON work_overlap_contract_engineers (engineer_id);
...
COMMENT ON COLUMN work_overlap_contract_engineers.engineer_id IS '기술자 ID';
...
CREATE TABLE IF NOT EXISTS work_overlap_contract_engineer_histories (
    ...
    contract_no VARCHAR(8) NOT NULL,
    before_engineer_id VARCHAR(50),
    after_engineer_id VARCHAR(50),
...
COMMENT ON COLUMN work_overlap_contract_engineer_histories.id IS '변경이력 ID';
COMMENT ON COLUMN work_overlap_contract_engineer_histories.contract_no IS '계약번호';
COMMENT ON COLUMN work_overlap_contract_engineer_histories.before_engineer_id IS '변경전 기술자 ID';
COMMENT ON COLUMN work_overlap_contract_engineer_histories.after_engineer_id IS '변경후 기술자 ID';
